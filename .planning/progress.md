...
## Session: 2026-06-03（补）

### 🔌 串口通信补入通信阶段

- 用户指出原计划 SerialPort 只有一行，这是漏洞
- 串口设备（扫码枪/电子秤/温控器/条码枪）是工厂标配，比网口设备更常见
- 通信阶段重排：
  - TCP基础(2周) → ⭐串口通信(1.5周) → Modbus TCP(2.5周) → S7+综合(4周)
- W19a：SerialPort 基础 + VSPD
- W19b：串口调试助手项目（选COM口/波特率/HEX发送/ASCII发送/接收/日志）
- 新增面试点：RS232 vs RS485、DataReceived在哪个线程触发
- 总时长不变(10周)，TCP从3周压缩到2周，串口从0变成1.5周
